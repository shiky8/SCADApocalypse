...
def fuzz(target, port=47808, payloads_file='bacnet_ip_payloads.txt', iterations=100):
    payloads = load_payloads(payloads_file)
    if not payloads:
        logger.error("No BACnet/IP payloads loaded")
        return
    ...
    try:
        ...
        logger.info(f"Connected to BACnet/IP on {target}:{port}")
        for i in range(min(iterations, len(payloads))):
            payload_hex = payloads[i]
            payload = binascii.unhexlify(payload_hex)
            logger.info(f"Trying BACnet/IP {payload = }")
            sock.send(payload)
            try:
        ...
        logger.error(f"BACnet/IP fuzzing error: {e}")
    finally:
        sock.close()
