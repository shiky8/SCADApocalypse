...
def fuzz(target, port=2404,payloads_file='iec104_payloads.txt', iterations=100):
    payloads = load_payloads(payloads_file)
    if not payloads:
        logger.error("No IEC-104 payloads loaded")
        return
    ...
    try:
        ...
        logger.info(f"Connected to IEC-104 on {target}:{port}")
        for i in range(min(iterations, len(payloads))):
            payload_hex = payloads[i]
            payload = binascii.unhexlify(payload_hex)
            logger.info(f"tring IEC-104 {payload = }")
            sock.send(payload)
            try:
        ...
        logger.error(f"IEC-104 fuzzing error: {e}")
    finally:
        sock.close()