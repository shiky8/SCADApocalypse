...
def fuzz(target, port=102,payloads_file='s7comm_payloads.txt', iterations=100):
    payloads = load_payloads(payloads_file)
    if not payloads:
        logger.error("No S7Comm+ payloads loaded")
        return
    ...
    try:
        ...
        logger.info(f"Connected to S7Comm+ on {target}:{port}")
        for i in range(min(iterations, len(payloads))):
            payload_hex = payloads[i]
            payload = binascii.unhexlify(payload_hex)
            logger.info(f"tring S7Comm {payload = }")
            sock.send(payload)
            try:
        ...
        logger.error(f"S7Comm+ fuzzing error: {e}")
    finally:
        sock.close()
