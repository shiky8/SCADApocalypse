...
def fuzz(target, port=4843, payloads_file='opc_ua_tls_payloads.txt', iterations=100):
    payloads = load_payloads(payloads_file)
    if not payloads:
        logger.error("No OPC UA TLS payloads loaded")
        return
    ...
    try:
        ...
        logger.info(f"Connected to OPC UA TLS on {target}:{port}")
        for i in range(min(iterations, len(payloads))):
            payload_hex = payloads[i]
            payload = binascii.unhexlify(payload_hex)
            logger.info(f"Trying OPC UA TLS {payload = }")
            sock.send(payload)
            try:
        ...
        logger.error(f"OPC UA TLS fuzzing error: {e}")
    finally:
        sock.close()
