...
def fuzz(target, port=5024, payloads_file='scpi_telnet_payloads.txt', iterations=100):
    payloads = load_payloads(payloads_file)
    if not payloads:
        logger.error("No SCPI Telnet payloads loaded")
        return
    ...
    try:
        ...
        logger.info(f"Connected to SCPI Telnet on {target}:{port}")
        for i in range(min(iterations, len(payloads))):
            payload_hex = payloads[i]
            payload = binascii.unhexlify(payload_hex)
            logger.info(f"Trying SCPI Telnet {payload = }")
            sock.send(payload)
            try:
        ...
        logger.error(f"SCPI Telnet fuzzing error: {e}")
    finally:
        sock.close()
