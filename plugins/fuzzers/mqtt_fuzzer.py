...
def fuzz(target, port=1883, payloads_file='mqtt_payloads.txt', iterations=100):
    payloads = load_payloads(payloads_file)
    if not payloads:
        logger.error("No MQTT payloads loaded")
        return
    ...
    try:
        ...
        logger.info(f"Connected to MQTT on {target}:{port}")
        for i in range(min(iterations, len(payloads))):
            payload_hex = payloads[i]
            payload = binascii.unhexlify(payload_hex)
            logger.info(f"Trying MQTT {payload = }")
            sock.send(payload)
            try:
        ...
        logger.error(f"MQTT fuzzing error: {e}")
    finally:
        sock.close()
