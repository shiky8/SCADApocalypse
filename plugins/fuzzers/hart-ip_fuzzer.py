...
def fuzz(target, port=5094, payloads_file='hart-ip_payloads.txt', iterations=100):
    payloads = load_payloads(payloads_file)
    if not payloads:
        logger.error("No HART-IP payloads loaded")
        return
    ...
    try:
        ...
        logger.info(f"Connected to HART-IP on {target}:{port}")
        for i in range(min(iterations, len(payloads))):
            payload_hex = payloads[i]
            payload = binascii.unhexlify(payload_hex)
            logger.info(f"Trying HART-IP {payload = }")
            sock.send(payload)
            try:
        ...
        logger.error(f"HART-IP fuzzing error: {e}")
    finally:
        sock.close()
