...
def fuzz(target, port=8883, payloads_file='mqtt_secure_payloads.txt', iterations=100):
    payloads = load_payloads(payloads_file)
    if not payloads:
        logger.error("No MQTT Secure payloads loaded")
        return
    ...
    try:
        ...
        logger.info(f"Connected to MQTT Secure on {target}:{port}")
        for i in range(min(iterations, len(payloads))):
            payload_hex = payloads[i]
            payload = binascii.unhexlify(payload_hex)
            logger.info(f"Trying MQTT Secure {payload = }")
            sock.send(payload)
            try:
        ...
        logger.error(f"MQTT Secure fuzzing error: {e}")
    finally:
        sock.close()
