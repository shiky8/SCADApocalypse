...
def vendor_lookup(vennum):
    return f"{VENDOR_IDS.get(vennum, 'Unknown Vendor')} ({vennum})"


def send_query(sock, ip, port, prop_id):
    object_id = 0x023FFFFF  # Object Type 8 (device), Instance Number 4194303
    request = struct.pack(
        ">BBHBBBxIBB",
        0x81, 0x0a, 0x0011, 0x01, 0x04, 0x00, 0x05, 0x01, 0x0c
    ) + struct.pack(">BI BB", 0x0c, object_id, 0x19, prop_id)
    sock.sendto(request, (ip, port))


def decode_field(response):
    if len(response) < 20:
        return None
    val = response[18]
    if val % 0x10 < 5:
        ...
        length = response[19] - 1
        offset = 20
    if len(response) < offset + 1 + length:
        return None
    charset = response[offset]
...
def receive_response(sock, prop, timeout):
    try:
...
def scan(target, port=47808, timeout=10):
    results = {}
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
            sock.bind(('', 0))
            for key, prop in QUERY_CODES.items():
                send_query(sock, target, port, prop)
                response = receive_response(sock, prop, timeout)
                print(f"[DEBUG] Raw response for {key}: {response}")

                if key == "vendor_id":
                    if isinstance(response, str) and response.isdigit():
                        results["Vendor ID"] = vendor_lookup(int(response))
                    elif isinstance(response, bytes) and len(response) >= 1:
                        results["Vendor ID"] = vendor_lookup(response[0])
                    else:
                        results["Vendor ID"] = "N/A"

                # if key == "vendor_id" and isinstance(response, bytes):
                #     results["Vendor ID"] = vendor_lookup(response[0])
                # elif key == "vendor_id" and isinstance(response, str) and response.isdigit():
                #     results["Vendor ID"] = vendor_lookup(int(response))
                # else:
                #     results[key.replace("_", " ").title()] = response or "N/A"
        print(f"{results = }")
        return results
