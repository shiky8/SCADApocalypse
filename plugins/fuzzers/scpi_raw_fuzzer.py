...
def fuzz(target, port=5025, payloads_file='scpi_raw_payloads.txt', iterations=100):
    payloads = load_payloads(payloads_file)
    if not payloads:
        logger.error("No SCPI Raw payloads loaded")
        return
    ...
    try:
        ...
        logger.info(f"Connected to SCPI Raw on {target}:{port}")
        for i in range(min(iterations, len(payloads))):
            payload_hex = payloads[i]
            payload = binascii.unhexlify(payload_hex)
            logger.info(f"Trying SCPI Raw {payload = }")
            sock.send(payload)
            try:
        ...
        logger.error(f"SCPI Raw fuzzing error: {e}")
    finally:
        sock.close()
