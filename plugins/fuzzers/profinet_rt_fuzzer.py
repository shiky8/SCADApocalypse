...
def fuzz(target, port=34962, payloads_file='profinet_rt_payloads.txt', iterations=100):
    payloads = load_payloads(payloads_file)
    if not payloads:
        logger.error("No Profinet RT payloads loaded")
        return
    ...
    try:
        ...
        logger.info(f"Connected to Profinet RT on {target}:{port}")
        for i in range(min(iterations, len(payloads))):
            payload_hex = payloads[i]
            payload = binascii.unhexlify(payload_hex)
            logger.info(f"Trying Profinet RT {payload = }")
            sock.send(payload)
            try:
        ...
        logger.error(f"Profinet RT fuzzing error: {e}")
    finally:
        sock.close()
