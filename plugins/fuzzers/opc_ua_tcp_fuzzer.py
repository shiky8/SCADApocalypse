...
def fuzz(target, port=4840, payloads_file='opc_ua_tcp_payloads.txt', iterations=100):
    payloads = load_payloads(payloads_file)
    if not payloads:
        logger.error("No OPC UA TCP payloads loaded")
        return
    ...
    try:
        ...
        logger.info(f"Connected to OPC UA TCP on {target}:{port}")
        for i in range(min(iterations, len(payloads))):
            payload_hex = payloads[i]
            payload = binascii.unhexlify(payload_hex)
            logger.info(f"Trying OPC UA TCP {payload = }")
            sock.send(payload)
            try:
        ...
        logger.error(f"OPC UA TCP fuzzing error: {e}")
    finally:
        sock.close()
