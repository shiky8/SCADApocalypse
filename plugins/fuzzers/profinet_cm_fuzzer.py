...
def fuzz(target, port=34964, payloads_file='profinet_cm_payloads.txt', iterations=100):
    payloads = load_payloads(payloads_file)
    if not payloads:
        logger.error("No Profinet CM payloads loaded")
        return
    ...
    try:
        ...
        logger.info(f"Connected to Profinet CM on {target}:{port}")
        for i in range(min(iterations, len(payloads))):
            payload_hex = payloads[i]
            payload = binascii.unhexlify(payload_hex)
            logger.info(f"Trying Profinet CM {payload = }")
            sock.send(payload)
            try:
        ...
        logger.error(f"Profinet CM fuzzing error: {e}")
    finally:
        sock.close()
